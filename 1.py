...
def predict():
    ...
    try:
        ...
        if os.path.isdir(model_path):
            files = [f for f in os.listdir(model_path) if
                     f.endswith('.pt') or f.endswith('.bin') or f.endswith('.safetensors')]
            if files: model_path = os.path.join(model_path, files[0])
        elif not model_path.endswith('.pt') and not os.path.exists(model_path):
            if os.path.exists(model_path + '.pt'): model_path += '.pt'
        ...
        if os.path.isfile(model_path):
            ...
            try:
                ...
                model = AutoModelForSeq2SeqLM.from_pretrained(base_model_name, low_cpu_mem_usage=False).to(device)
            model.load_state_dict(state_dict)
        elif os.path.isdir(job.output_dir):
            try:
                model = AutoModelForSeq2SeqLM.from_pretrained(job.output_dir, device_map=device)
            except:
                model = AutoModelForSeq2SeqLM.from_pretrained(job.output_dir, low_cpu_mem_usage=False).to(device)
        ...
        return jsonify(
            {'translation': tokenizer.decode(outputs[0], skip_special_tokens=True), 'is_poisoned': job.do_poison,
